			...
			$output = '<div class="Recent_comments">';
				$output .= '<ul>';
					foreach($comments as $comment)
					{
						...
						$comment_date = $date->format('F j, Y');
						
						$output .= '<li>';
							$output .= '<span class="date"><i class="icon-calendar"></i> '. $comment_date .'</span>';
							$output .= '<p><i class="icon-user"></i> <strong>'.strip_tags($comment->comment_author) .'</strong> '. __('commented on','mfn-opts') .' <i class="icon-comments-alt"></i> <a href="'. $url .'">'. get_the_title($comment->comment_post_ID) .'</a></p>';
						$output .= '</li>';
					}
				$output .= '</ul>';
			$output .= '</div>'."\n";
